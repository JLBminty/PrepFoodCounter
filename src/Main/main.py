...
class MenuItemFrame():
    ...
    def __init__(self,item:MenuItem):
        ...
        #row's add button
        self.add_bttn = tk.Button(self.the_frame,text='+', bg='green', fg='white', command=add_btn_click)
        self.add_bttn.pack(side=LEFT, padx=5)
        #row's subtract button
        self.minus_bttn = tk.Button(self.the_frame,text='-', bg='red', fg='white', command=minus_btn_click)
        self.minus_bttn.pack(side=LEFT, padx=5)

        menu_gui_dict.update({item:self.the_frame})
...
#    print(type(item))
    menu_gui_dict.get(item).pack(padx=20, pady=5)
